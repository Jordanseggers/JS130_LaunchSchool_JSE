let SumOfMultiples = require('./08_SumOfMultiples.js');
...
describe("sum of multiples", () => {
  ...
  test("sum to one hundred", () => {
    expect(SumOfMultiples.to(100)).toEqual(2318);
  });

  test("sum to one thousand", () => {
    expect(SumOfMultiples.to(1000)).toEqual(233168);
  });
  ...
  test("configurable 5 6 8 to 150", () => {
    ...
    expect(obj.to(150)).toEqual(4419);
  });

  test("configurable 43 47 to 10,000", () => {
    let obj = new SumOfMultiples(43, 47);
    expect(obj.to(10000)).toEqual(2203160);
  });
});